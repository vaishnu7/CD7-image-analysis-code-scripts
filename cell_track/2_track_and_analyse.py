...
def measure(cell_mask, nuc_mask, ch1, ch2, ch3, chosen_lbl, r1, r2, c1, c2):
    ...
    def mean_px(arr, mask):
        return float(arr[mask].mean()) if mask.any() else nan
    ...
    print(f"  bg_gfp={bg1:.0f}  "
          f"raw_nuc_mean={float(ch1c[nuc_px].mean()) if nuc_px.any() else 'EMPTY':.0f}  "
          f"raw_cyto_mean={float(ch1c[cyto_px].mean()) if cyto_px.any() else 'EMPTY':.0f}  "
          f"nuc_px_count={nuc_px.sum()}  cyto_px_count={cyto_px.sum()}")

    gfp_tot  = mean_px(ch1c, cell_px) - bg1
    gfp_nuc  = mean_px(ch1c, nuc_px)  - bg1
    gfp_cyto = mean_px(ch1c, cyto_px) - bg1
    mch_tot  = mean_px(ch2c, cell_px) - bg2
    mch_nuc  = mean_px(ch2c, nuc_px)  - bg2
    mch_cyto = mean_px(ch2c, cyto_px) - bg2
    h2b_nuc  = mean_px(ch3c, nuc_px)  - bg3
    h2b_cyto = mean_px(ch3c, cyto_px) - bg3
